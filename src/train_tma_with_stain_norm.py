...
class PaddingAndCenter(A.ImageOnlyTransform):
    def __init__(self, pad_size=(3584, 3584), p=0.5) -> None:
        super().__init__()
        self.prob = p
        self.pad_size = pad_size

    def apply(self, image, copy=True, **params):
            # Get the current size of the image
        image_height, image_width = image.shape[:2]

        max_width = max(image_width, self.pad_size[0])
        max_height = max(image_height, self.pad_size[1])

        pad_left, pad_top = 0, 0
        if image_width > self.pad_size[0]:
            pad_left = (image_width - self.pad_size[0]) // 2
        if image_height > self.pad_size[1]:
            pad_top = (image_height - self.pad_size[1]) // 2

        image = image[pad_top:pad_top + self.pad_size[1], pad_left:pad_left + self.pad_size[0]]
        image_height, image_width = image.shape[:2]

        pad_left = (self.pad_size[0] - image_width) // 2
        pad_top = (self.pad_size[1] - image_height) // 2

        template_image = np.ones((self.pad_size[1], self.pad_size[0], 3), dtype=np.uint8) * 255
        template_image[pad_top:pad_top + image_height, pad_left:pad_left + image_width] = image

        return template_image

    def get_transform_init_args_names(self):
        return {"pad_size": self.pad_size}


class WhiteEllispe(A.ImageOnlyTransform):
    def __init__(self, p=0.5) -> None:
        super().__init__()
        self.prob = p
        self.angle = 0
        self.radius_range = (200, 265)
        self.center_coord_range = (-10, 10)

    def apply(self, image, copy=True, **params):

        image_size = image.shape[:2]
        center_x = random.randint(self.center_coord_range[0], self.center_coord_range[1])
        center_y = random.randint(self.center_coord_range[0], self.center_coord_range[1])
        center_coord = (image_size[1] // 2 + center_x, image_size[1] // 2 + center_y)

        radius_gap = self.radius_range[1] - self.radius_range[0]
        radius_a = self.radius_range[0] + random.randint(1, radius_gap)
        radius_b = self.radius_range[0] + random.randint(1, radius_gap)

        # mask = np.zeros(image_size, dtype=np.uint8)
        mask = np.zeros_like(image)
        axes_length = (radius_a, radius_b)
        ...
        cv2.ellipse(img=mask,
                    ...
                    thickness=-1)
        others = 255 - mask
        augmented_image = image * (mask // 255) + others
    ...
    def get_transform_init_args_names(self):
        return {
            ...
            "center_coord_range": self.center_coord_range,
        }
...
class StainNormAndCancerSubtypeClassifier(nn.Module):
    def __init__(self, cfg):
        super(StainNormAndCancerSubtypeClassifier, self).__init__()
        self.cfg = cfg
        ...
        self.stain_norm_model = StainNet()
        self.stain_norm_model.load_state_dict(torch.load(cfg.stain_norm_model_ckpt))
        self.stain_norm_model.eval()
    ...
    def forward(self, images, labels=None):
        with torch.no_grad():
            images = self.stain_norm_model(images)
...
def train(cfg):
    ...
    train_datasets = [
        UBCDataset(ele['df'], class2class_idx, transforms=cfg.data_transforms["train"]) for ele in train_dicts
    ]
    ...
    model_module.model = StainNormAndCancerSubtypeClassifier(cfg)
        ...
        "train": A.Compose(
            [
                ...
                A.HueSaturationValue(
                    hue_shift_limit=10,  # Increased limit for more pronounced color shifts
                    sat_shift_limit=0.25,  # Slightly higher saturation shift
                    val_shift_limit=0.25,  # Slightly higher value shift
                    p=0.5,
                ),
                # A.ChannelShuffle(p=0.1),  # Randomly shuffle channels to change color perspectives
                ...
                # WhiteEllispe(p=0.5),
                A.CLAHE(clip_limit=4, tile_grid_size=(8, 8), p=0.3),  # Apply CLAHE for enhancing contrast
            ],
            p=1.0,
        ),
        "valid": A.Compose([
                # PaddingAndCenter(pad_size=pad_size, p=1.0),
                A.Resize(*image_size),
            ],
            p=1.0,
        )
